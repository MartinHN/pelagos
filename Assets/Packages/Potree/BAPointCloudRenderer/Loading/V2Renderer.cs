...
namespace BAPointCloudRenderer.Loading
{
    ...
    class V2Renderer : AbstractRenderer
    {
        ...
        public V2TraversalThread traversalThread;
        public V2LoadingThread loadingThread;
        public V2Cache cache;
        public List<Node> rootNodes;   //List of root nodes of the point clouds
        ...
        public V2Renderer(AbstractPointCloudSet pcset, int minNodeSize, int grandMaxLevel, double maxDistance, int maxLevel, uint pointBudget, uint nodesLoadedPerFrame, uint nodesGOsperFrame, List<Camera> cameras, MeshConfiguration config, uint cacheSize)
        {
            this.pcset = pcset;
            rootNodes = new List<Node>();
            this.cameras = new List<Camera>(cameras);
            this.config = config;
            cache = new V2Cache(cacheSize);
            loadingThread = new V2LoadingThread(cache);
            loadingThread.Start();
            traversalThread = new V2TraversalThread(pcset.gameObject, this, loadingThread, rootNodes, minNodeSize, grandMaxLevel, maxDistance, maxLevel, pointBudget, nodesLoadedPerFrame, nodesGOsperFrame, cache);
        ...
        public void Update()
        {
            ...
            traversalThread.SetNextCamerasData(this.cameras, pcset.transform);
