...
namespace BAPointCloudRenderer.Loading
{
    ...
    class CloudLoader
    {
        ...
        public static PointCloudMetaData LoadMetaData(string cloudPath, string fallbackCachePath, bool moveToOrigin = false)
        {
            string jsonfile = "";
            //Debug.Log(cloudPath);
            ...
            if (isCloudOnline)
            {
                try
                {
                    ...
                    reader.Close();
                    if (!Directory.Exists(fallbackCachePath))
                    {
                        Directory.CreateDirectory(fallbackCachePath);
                    }
                    File.WriteAllText(fallbackCachePath + "/cloud.js", jsonfile);
                }
                catch
                {
                    Debug.LogWarning("cloud is offline");
                    isCloudOnline = false;
                    cloudPath = fallbackCachePath;
                    jsonfile = "";
                }
            ...
            if (isCloudOnline)
            {
                ...
                metaData.cloudPath = fallbackCachePath;
            }
            else
            {
        ...
        /// </summary>
        public static bool LoadPointsForNode(Node node)
        {
            string dataRPath = node.MetaData.octreeDir + "/r/";
            return LoadPoints(dataRPath, node.MetaData, node);
        }
        /// <summary>
        /// for Potree v2
        /// </summary>
        ...
        /// <param name="metaData"></param>
        /// <param name="node"></param>
        private static bool LoadPoints(string dataRPath, PointCloudMetaData metaData, Node node)
        {
        ...
        public static Queue<Node> getPreloadableNodes(Node rootNode, PointCloudMetaData metadata)
        {
            Queue<Node> outQ = new Queue<Node>();
            if (metadata.cloudUrl == null)
            {
                Debug.LogWarning("cant preload if offline");
                return outQ;
            }
            // var oldUrl = metadata.cloudUrl;
            // metadata.cloudUrl = null; // prevent fetching them online

            string dataRPath = rootNode.MetaData.octreeDir + "/r/";
            Debug.Log("pre : " + dataRPath + " , " + metadata.cloudPath);
            appenPreloadableNode(dataRPath, rootNode, metadata, outQ);
            // metadata.cloudUrl = oldUrl;

            return outQ;

        }

        static void appenPreloadableNode(string dataRPath, Node node, PointCloudMetaData metaData, Queue<Node> outQ)
        {
            // if (outQ.Count > 100) return;
            ...
            {
                if (node.HasChild(i))
                    appenPreloadableNode(dataRPath, node.GetChild(i), metaData, outQ);
            }
