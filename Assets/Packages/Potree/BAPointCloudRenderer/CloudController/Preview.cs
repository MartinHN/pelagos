...
namespace BAPointCloudRenderer.CloudController
{
    ...
    public class Preview : MonoBehaviour
    {
        ...
        public void KillAll()
        {
            if (loadingThread != null) { loadingThread.Abort(); loadingThread = null; }
            _loaders = null;
        ...
        private void LoadBoundingBoxes()
        {
            ...
            foreach (PointCloudLoader loader in _loaders)
            {
                ...
                if (_showPoints)
                {
                    ...
                    Node root = CloudLoader.LoadHierarchyOnly(metaData);
                    Queue<Node> toParse = new Queue<Node>();

                    for (int c = 0; c < 8; c++)
                    {
                        if (root.HasChild(c))
                        {
                            toParse.Enqueue(root.GetChild(c));
                        }
                    }
                    int i = 0;
                    int total = rootNode.PointCount;
                    while (toParse.Count > 0)
                    {
                        Node n = toParse.Dequeue();
                        Debug.Log("loading child " + i + n);
                        CloudLoader.LoadPointsForNode(n);
                        total += n.PointCount;
                        Debug.Log("got " + n.PointCount + " points : total : " + total);
                        if (total > _pointBudget)
                        {
                            break;
                        }
                        _nodes.Add(n);
        ...
        private void CreateMesh()
        {
            ...
            foreach (Tuple<PointCloudLoader, Vector3[], Color[]> cloud in data)
            {
                ...
                processed++;
                Debug.Log("generating unity obj " + processed + " / " + data.Count);
                GameObject go = new GameObject("Preview: " + cloud.Item1.cloudPath + processed);
                ...
                var path = assetMeshPath + "previewMesh" + processed + ".asset";
        ...
        private List<Tuple<PointCloudLoader, Vector3[], Color[]>> ChoosePoints()
        {
            ...
            while ((currentPointCount < finalsumpoints) && (remainingNodeCount > 0))
            {
            ...
            foreach (Node n in _nodes)
            {
                ...
                result.Add(new Tuple<PointCloudLoader, Vector3[], Color[]>(_loaders[n.Parent != null ? 0 : j], filteredVertices, filteredColors));
                ++j;
            }
            return result;
        }
